			...
			lessLoader(),
			{
				...
				setup(build) {
					build.onEnd((result) => {
						...
						for (let soundId in soundsData) {
							if (Array.isArray(soundsData[soundId])) {
								for (let sound of soundsData[soundId]) {
									...
									js = js.replace(
										sound.sound,
										`data:audio/${sound.sound.split(".").pop()};base64,${base64}`
									);
								}
								continue;
							}
							...
							const base64 = audioFile.toString("base64");
							js = js.replace(
								soundsData[soundId].sound,
								`data:audio/${soundsData[soundId].sound.split(".").pop()};base64,${base64}`
							);
						}
							...
							if (link.rel == "stylesheet") {
								...
								for (const font of fonts) {
									...
									contents = contents.replace(
										font[0],
										`url("data:application/font-${font[2]};base64,${base64}")`
									);
								}
						...
						dom.window.document.body.appendChild(dom.window.document.createElement("script")).textContent =
							contentScriptText.replace("__CURRENT_SHA__", '"' + sha + '"');

						dom.window.document.body.appendChild(dom.window.document.createElement("script")).textContent =
							js;
						dom.window.document.head.appendChild(dom.window.document.createElement("style")).textContent =
							css;
