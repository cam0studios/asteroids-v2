...
export function levelUp() {
	...
	let choices = [];
	playerUpgrades
		.filter((upgrade) => upgrade.times < upgrade.max && unlocks.playerUpgrades[upgrade.id])
		.forEach((upgrade) => {
			for (let _ = 0; _ < upgrade.weight; _ += 0.05) choices.push({ type: 0, val: upgrade });
		});
	player.weapons.forEach((weapon, weaponI) => {
		weapon.upgrades
			.filter((upgrade) => {
				return (
					upgrade.times < upgrade.max &&
					!weapon.upgrades.filter((x) => x.times > 0).some((x) => x.incompatible?.includes(upgrade.id)) &&
	...
	if (chosen.length == 0)
		chosen.push({
			type: -1,
			val: { name: "Healing", desc: "Heal 40 HP", func: () => (player.hp += 40), max: 1, times: 0 },
		});

	function getDescription(option) {
		if (typeof option.val.desc == "string") return option.val.desc;
		if (Array.isArray(option.val.desc))
			return option.val.desc[option.val.times] || option.val.desc[option.val.desc.length - 1];
	}

	chosen.forEach((option, i) => {
		content +=
			`<button id="option${i}" class="upgrade-choice ${getRarity(option.val.weight)}"><h2>${
				option.type == 1 ? `${player.weapons[option.i].name} - ` : ""
			}${option.val.name}</h2><p>${getDescription(option)}</p>` +
			(option.type != 2 ? `<p>${option.val.times}/${option.val.max}</p>` : "") +
			"</button>";
	});
